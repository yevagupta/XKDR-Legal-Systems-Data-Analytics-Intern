...
df = df[['hearing_id', 'matter_id', 'hearing_date']]
...
print(df1['case_status'].unique())
# Ensure hearing_date is datetime
df1['hearing_date'] = pd.to_datetime(df1['hearing_date'], errors='coerce')

# Drop rows with missing hearing dates (to avoid issues)
df1 = df1.dropna(subset=['hearing_date'])

# Sort so latest hearing is last per matter
df1_sorted = df1.sort_values(['matter_id', 'hearing_date'])

# Keep only the last hearing per matter_id
latest_hearings = df1_sorted.drop_duplicates(subset='matter_id', keep='last')
...
yearly_counts.to_csv("yearly_counts_with_overlaps_BHC.csv", index=False)

# Export 2: Year-wise counts based on FIRST appearance only
first_year_counts.to_csv("yearly_counts_first_appearance_BHC.csv", index=False)

# Export 3: Final case status (from last hearing per matter)
status_df = status_counts.reset_index()
status_df.columns = ['case_status', 'count']
status_df.to_csv("final_case_status_summary_BHC.csv", index=False)
